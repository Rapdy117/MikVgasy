...
?>
<!DOCTYPE html>
<html lang="fr" class="reports-page">
<head>
    <meta charset="UTF-8">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const reportsExportActions = document.getElementById('reportsExportActions');
    const reportsAnalysisTab = document.getElementById('reports-analysis-tab');
    const reportsDetailsTab = document.getElementById('reports-details-tab');
        ...
        new Chart(modeCanvas.getContext('2d'), {
            ...
            options: {
                ...
                plugins: {
                    legend: {
                        position: 'bottom',
                        labels: { color: '#e5eefc', boxWidth: 10 },
                    },
                },
            },
        });
    ...
    const applyReportFilters = () => {
        ...
        reportTableBody.querySelectorAll('tr').forEach((row) => {
            ...
            row.style.display = (matchesText && matchesDay && matchesMonth && matchesYear) ? '' : 'none';
        });
    };

    [searchInput, dayFilter, monthFilter, yearFilter].forEach((element) => {
        if (!element) {
            return;
        }
    ...
    const updateExportVisibility = (activeTabId) => {
        if (!reportsExportActions) {
            return;
        }
        const isDetails = activeTabId === 'reports-details-tab';
        reportsExportActions.classList.toggle('is-hidden', !isDetails);
    };

    updateExportVisibility(reportsAnalysisTab?.classList.contains('active') ? 'reports-analysis-tab' : 'reports-details-tab');

    [reportsAnalysisTab, reportsDetailsTab].forEach((tab) => {
        tab?.addEventListener('shown.bs.tab', (event) => {
            updateExportVisibility(event.target.id);
        });
    });
});
</script>
